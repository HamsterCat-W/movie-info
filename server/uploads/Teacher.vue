    ...
    >
      ...
      >
        ...
        <a-form-model-item label="职称">
          ...
          <a-select @change="handleSelectChangeTitle" v-model="form.title">
            <a-select-option
        ...
        <a-form-model-item label="学院">
          <a-select @change="handleSelectChangeSub" v-model="form.subOrg_name">
            <a-select-option
...
import FeedbackVue from "../../../teacher/course/Feedback.vue";
export default {
  components: {
    batchAddTeacher,
  },
  data() {
    ...
    return {
      ...
      //学院列表
      titles: [
        { _id: "1", title: "professor" },
        { _id: "2", title: "vice-professor" },
        { _id: "3", title: "lecturer" },
      ],
      colleges: [],
      teacherList: [],
    };
  ...
  computed: {
    ...mapState({
      ...
      teachers: (state) => state.admin.teacherList,
      userState: (state) => state.admin.userState,
    }),
  },
  mounted() {
    this.getTeacherList();
  ...
  methods: {
    ...
    async getTeacherList() {
      ...
      try {
        ...
        // this.$store.commit("admin/getTeacherList", data.teachers);
      } catch (err) {
        console.log(err);
      }
    },
